...
public final class ClickGui
{
	...
	public void init(HackList hax)
	{
		...
		// Add OtherFeatures to the Other window
		Window otherWindow = windowMap.get(Category.OTHER);
		for(OtherFeature otf : ForgeWurst.getForgeWurst().getOtfs().getAllOtfs())
			otherWindow.add(new OtherFeatureButton(otf));
	...
	/**
	 * Handles key presses for keybind assignment.
	 * Returns true if a key was consumed by a waiting HackButton.
	 */
	public boolean handleKeyPress(int keyCode)
	{
		for(Window window : windows)
		{
			for(int i = 0; i < window.countChildren(); i++)
			{
				...
				if(child instanceof HackButton)
				{
					HackButton button = (HackButton)child;
					if(button.handleKeyPress(keyCode))
						return true;
				}
			}
		}
